...
for (strategy in trading_strategies){
  for (instrument in instruments){
    for (file in files){
      relative_path <- file.path("..", "..", "..","Data", "Results", "TradingStrategies", strategy, instrument, file)
      ...
      relative_path <- file.path("..", "..", "..","Data", "Results", "StatisticalTests", "Sharpe",strategy, instrument, new_filename)
...
#indices=(which(StrategyAETF10Y12M < 0.35, arr.ind = TRUE))
#length(indices)
